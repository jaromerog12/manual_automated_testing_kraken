  ...
  (async () => {
    ...
    for(b of browsers){
     ...
     const browser = await playwright[b].launch({headless: headlessFlag, viewport: {width:viewportWidth, height:viewportHeight}});
      ...
      await recursiveExploration(page, baseUrl, 0, -1, browser); 
 ...
 */
async function recursiveExploration(page, link, depth, parentState, browser){
  ...
  if(depth === 0){
    await page.waitForSelector('#ember12'); 
  }
  ...
  if(!!body) {
    ...
    if(link.includes(baseUrl)){ //Only explore pages of the specified domain
      ...
      for(const newLink of links){
        await recursiveExploration(page, newLink, depth+1, currentState, browser);
      }
    }
...
async function fillInput(elementHandle, page){
  ...
  if(type === 'text'){
    ...
    console.log('entre '+elementHandle);
    elementHandle.click();
  }
}
